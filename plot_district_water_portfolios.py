...
def subplots_dataset_comparison(irrigation_district, sum_crop_types, num , fig, ax  ): 
    ...
    y_min_water_demand = data_2016.minimum_water_demand_for_year /1000 # converted to TAF
    y_normal_water_demand = data_2016.water_demand_with_2010_AW_values / 1000 # converted to TAF
    ax[row, column].hlines(y_normal_water_demand, -1, 2, colors = '#6baed6', label = 'estimated annual water demand')
    ax[row, column].hlines(y_min_water_demand, -1, 2, colors = '#08519c', label = 'minimum water demand')
    ...
    ax[row, column].set_title(str(irrigation_district))
...
fig.text(0.5, 0.04, 'Year Type', ha='center')
fig.text(0.04, 0.5, 'Annual water volume [TAF]', va='center', rotation='vertical')
